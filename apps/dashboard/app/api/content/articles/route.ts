...
import { getAllArticles, createArticle } from '@siteforge/db';
...
export async function GET(req: NextRequest) {
  ...
  const articles = await getAllArticles(supabase as any, siteId);
...
export async function POST(req: NextRequest) {
  ...
  if (!(await isOwner(supabase, body.site_id, user.id)))
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const article = await createArticle(createServiceClient(), body);
