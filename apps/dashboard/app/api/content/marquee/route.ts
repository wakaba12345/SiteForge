...
import { getAllMarqueeItems, createMarqueeItem } from '@siteforge/db';
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(await getAllMarqueeItems(supabase as any, siteId));
}
...
export async function POST(req: NextRequest) {
  ...
  if (!(await isOwner(supabase, body.site_id, user.id)))
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const item = await createMarqueeItem(createServiceClient(), body);
