...
function VisualPreview({ preview, siteName }: { preview: PreviewData; siteName: string }) {
  ...
  return (
    <div style={{ ...vars, background: 'var(--bg)', fontFamily: '"Noto Sans TC", sans-serif', fontSize: 12 }}
      ...
      {/* Process */}
      {(preview.process?.length ?? 0) > 0 && (
        <div style={{ background: 'var(--bg)', borderTop: '1px solid var(--bd)', padding: '18px 20px' }}>
          <div style={{ textAlign: 'center', marginBottom: 14 }}>
            <div style={{ width: 24, height: 3, background: `linear-gradient(90deg, var(--p), var(--a))`, margin: '0 auto 8px', borderRadius: 99 }} />
            <div style={{ color: 'var(--tx)', fontFamily: 'var(--hf)', fontWeight: 700, fontSize: 13 }}>
              {preview.processTitle || '服務流程'}
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(preview.process!.length, 4)}, 1fr)`, gap: 8 }}>
            {preview.process!.map((step, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '6px 4px' }}>
                <div style={{
                  width: 26, height: 26, borderRadius: '50%',
                  background: `linear-gradient(135deg, ${c.primary ?? '#1a2f4a'}, ${c.accent ?? '#b8973a'})`,
                  color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 9, fontWeight: 900, marginBottom: 5, fontFamily: 'var(--hf)',
                }}>{String(i + 1).padStart(2, '0')}</div>
                <div style={{ color: 'var(--tx)', fontWeight: 700, fontSize: 10, marginBottom: 2 }}>{step.title}</div>
                <div style={{ color: 'var(--ts)', fontSize: 9, lineHeight: 1.4 }}>{step.description.slice(0, 28)}{step.description.length > 28 ? '…' : ''}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      {preview.ctaTitle && (
        <div style={{ background: c.primary ?? '#1a2f4a', padding: '20px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 70% 50%, ${c.accent ?? '#b8973a'}, transparent 60%)`, opacity: 0.15 }} />
          <div style={{ position: 'relative' }}>
            <div style={{ width: 20, height: 3, background: 'rgba(255,255,255,0.35)', margin: '0 auto 8px', borderRadius: 99 }} />
            <div style={{ color: '#fff', fontFamily: 'var(--hf)', fontWeight: 700, fontSize: 14, marginBottom: 5, lineHeight: 1.4 }}>{preview.ctaTitle}</div>
            <div style={{ color: 'rgba(255,255,255,0.65)', fontSize: 9, marginBottom: 10 }}>{preview.ctaDescription}</div>
            <div style={{ display: 'inline-block', background: c.accent ?? '#b8973a', color: '#fff', padding: '4px 14px', borderRadius: radius, fontSize: 10, fontWeight: 700 }}>
              {preview.ctaButtonText || '立即聯絡'} →
            </div>
          </div>
        </div>
      )}
...
export default function GeneratePage({ params }: { params: { siteId: string } }) {
  ...
  const DEVICES = [
    { key: 'desktop', label: '桌機', icon: '🖥', width: '100%' },
    { key: 'tablet', label: '平板', icon: '⬜', width: '768px' },
    { key: 'mobile', label: '手機', icon: '📱', width: '390px' },
  ] as const;
    ...
    <div className="flex flex-col h-full max-w-3xl mx-auto">
      ...
      {tab === 'quickbuild' && (
        <div className="flex flex-col gap-5">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
                  ...
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 bg-slate-50 gap-3 flex-wrap">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-xs text-slate-400 font-mono truncate">{siteUrl}</span>
                    </div>

                    {/* Device switcher */}
                    <div className="flex items-center gap-1 bg-white rounded-lg border border-slate-200 p-0.5 shrink-0">
                      {DEVICES.map((d) => (
                        <button
                          key={d.key}
                  ...
                  {/* iframe with device width */}
                  <div className="bg-slate-100 flex justify-center overflow-auto"
                    style={{ minHeight: 560, padding: previewDevice === 'desktop' ? 0 : '12px' }}>
                    <div
                      className="bg-white shadow-md overflow-hidden transition-all duration-300"
                      style={{
                        width: DEVICES.find(d => d.key === previewDevice)!.width,
                        minHeight: 560,
                        borderRadius: previewDevice === 'desktop' ? 0 : 8,
                      }}
                    >
                      <iframe
                        key={iframeKey}
          ...
          <div className="text-xs text-slate-500 bg-slate-50 rounded-xl p-4 space-y-2 leading-relaxed">
            <p><strong className="text-slate-700">🔗 網址是永久的：</strong>部署後的網址不會變，每次套用內容都更新同一個網址。</p>
            <p><strong className="text-slate-700">✅ 套用內容 = 不需重新部署：</strong>文章、消息、跑馬燈、主題設定套用後透過 ISR 自動更新，<strong>看到舊的請等 1-2 分鐘再重新整理</strong>。</p>
            <p><strong className="text-slate-700">🚀 只有改程式碼才需重新部署：</strong>例如新增功能、修改版面結構。平常操作完全不需要。</p>
            <p><strong className="text-slate-700">⚠️ 套用會覆蓋：</strong>套用後會取代現有文章、消息和跑馬燈，請確認後再套用。</p>
          </div>
        </div>
      )}
